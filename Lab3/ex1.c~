...
int main (int argc, void **argv)
{
  ...
  if ((fp = fopen(nume_fis, "ab")) == NULL)
  {
   ...
   /*Sesiune grafica */
   gplot_ctrl *hgp;
   if ((hgp = gplot_init()) == NULL)
   {
     printf("Nu pot initializa sesiunea grafica \n\n");
     exit(1);
  }
  gplot_set_xlabel(hgp, "x (div)");
  gplot_set_ylabel(hgp, "Jo(x)");
  
  gplot_cmd(hgp, "set term pdf");
  gplot_cmd(hgp, "set output \"grafic.pdf\"");
  
  strcpy (nume_fis, argv[1]);
  strcat (nume_fis, ".txt");
  gplot_cmd(hgp, "plot '%s' using 1:2", nume_fis); /* X - coloana 1, Y - coloana 2 */
//  sleep(20);
  gplot_close(hgp);
